...
public class AndroidMeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(savedInstanceState==null) {
            ...
            bodyFragment = new MiddleBodyPart();

            bodyFragment.setmBImageIds(AndroidImageAssets.getBodies());
            bodyFragment.setmBindex(1);

            fragmentManager.beginTransaction().add(R.id.body_container, bodyFragment).commit();

            legFragment = new LegPartFragment();

            legFragment.setmLImageIds(AndroidImageAssets.getLegs());
            legFragment.setmLindex(1);
